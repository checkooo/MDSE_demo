...
def SelfAttentionGradientGreedyAttack(device, epsMax, numSteps, epsStep, modelListPlus, modelLists, coefficientArray, dataLoader, clipMin,
                                clipMax):
    # Basic graident variable setup
    xClean, yClean = DMP.DataLoaderToTensor(dataLoader)
    xAdv = xClean.to(device).detach()  # Set the initial adversarial samples
    ...
    for i in range(0, numSteps):
        ...
        for m in range(0, len(modelListPlus)):
            currentModelPlus = modelListPlus[m]
            currentModelList = modelLists[m]
            # First get the gradient from the model
            # dataLoaderCurrent = currentModelPlus.formatDataLoader(dataLoaderCurrent)
            if modelListPlus[m].modelName.find("SNN") >= 0:
                xGradientCurrent = FGSMGreedyGradient(device, dataLoaderCurrent, currentModelPlus, currentModelList)
            else:
                xGradientCurrent = FGSMNativeGradient(device, dataLoaderCurrent, currentModelPlus)
...
def SelfAttentionGradientAttackProtoGreedyProfiling(device, epsMax, epsStep, numSteps, modelListPlus, modelLists, estlists, dataLoader, clipMin, clipMax,
                                     alphaLearningRate, fittingFactor, advLoader=None, numClasses=10):
    print("Using hard coded experimental function not advisable.")
    # Basic graident variable setup
    xClean, yClean = DMP.DataLoaderToTensor(dataLoader)
    ...
    for i in range(0, numSteps):
        ...
        dFdX = torch.zeros(numSamples, xShape[0], xShape[1],
                           xShape[2])  # Change to the math here to take in account all objecitve functions
        # dFdX = torch.zeros(len(modelListPlus), numSamples, xShape[0], xShape[1], xShape[2])
        attmapArray = torch.ones(size=(len(modelListPlus), numSamples, xShape[0], xShape[1], xShape[2])) #put in dummy attention map, only fill in if ViT model
        # Go through each model and compute dC/dX
        for m in range(0, len(modelListPlus)):
            # dataLoaderCurrent = modelListPlus[m].formatDataLoader(dataLoaderCurrent)
            if modelListPlus[m].modelName.find("SNN") >= 0:
                dCdXTemp = FGSMGreedyGradientProfiling(device, dataLoaderCurrent, modelListPlus[m], modelLists[m], estlists[m])
            else:
                dCdXTemp = FGSMNativeGradient(device, dataLoaderCurrent, modelListPlus[m])
            if modelListPlus[m].modelName.find("ViT") >= 0:
                attmap = GetAttention(dataLoaderCurrent, modelListPlus[m])
                attmap = torch.nn.functional.interpolate(attmap, size=(xShape[1], xShape[2]))
                attmapArray[m] = attmap
                dCdX[m] = dCdX[m] * attmapArray[m]
            # Resize the graident to be the correct size and save it
            dCdX[m] = torch.nn.functional.interpolate(dCdXTemp, size=(xShape[1], xShape[2]))
            # Now compute the inital adversarial example with the base alpha
        xGradientCumulative = torch.zeros(numSamples, xShape[0], xShape[1], xShape[2])
        ...
        for m in range(0, len(modelListPlus)):
            dFdX = dFdX + torch.nn.functional.interpolate(
                ...
                size=(xShape[1], xShape[2]))
        # Compute dX/dAlpha
        dXdAlpha = dXdAlphaCompute(fittingFactor, epsStep, alpha, dCdX, len(modelListPlus), numSamples, attmapArray, xShape)
        # Compute dF/dAlpha = dF/dx * dX/dAlpha
        dFdAlpha = torch.zeros(len(modelListPlus), numSamples, xShape[0], xShape[1], xShape[2])
        for m in range(0, len(modelListPlus)):
            # dFdAlpha = dFdX * dXdAlpha[m]
        ...
        if i == 10 or i == 20 or i == 30 or i == 40 or i == 60:
            accArrayProtoSAGA = torch.zeros(
                numSamples).to(device)  # Create an array with one entry for ever sample in the dataset
            for idx in range(0, len(modelListPlus)):
                accArray = modelListPlus[idx].validateDA(dataLoaderCurrent)
...
def SelfAttentionGradientAttackProtoGreedy(device, epsMax, epsStep, numSteps, modelListPlus, modelLists, dataLoader, clipMin, clipMax,
                                     alphaLearningRate, fittingFactor, advLoader=None, numClasses=10):
    print("Using hard coded experimental function not advisable.")
    # Basic graident variable setup
    xClean, yClean = DMP.DataLoaderToTensor(dataLoader)
    ...
    for i in range(0, numSteps):
        ...
        for m in range(0, len(modelListPlus)):
            # dataLoaderCurrent = modelListPlus[m].formatDataLoader(dataLoaderCurrent)
            if modelListPlus[m].modelName.find("SNN") >= 0:
                dCdXTemp = FGSMGreedyGradient(device, dataLoaderCurrent, modelListPlus[m], modelLists[m])
            else:
                dCdXTemp = FGSMNativeGradient(device, dataLoaderCurrent, modelListPlus[m])
            if modelListPlus[m].modelName.find("ViT") >= 0:
                attmap = GetAttention(dataLoaderCurrent, modelListPlus[m])
...
# Compute dX/dAlpha for each model m
def dXdAlphaCompute(fittingFactor, epsStep, alpha, dCdX, numModels, numSamples, attmapArray, xShape):
    # Allocate memory for the solution
    dXdAlpha = torch.zeros(numModels, numSamples, xShape[0], xShape[1], xShape[2])
    innerSum = torch.zeros(numSamples, xShape[0], xShape[1], xShape[2])
    # First compute the inner summation sum m=1,...M: a_{m}*dC/dX_{m}
    for m in range(0, numModels):
        innerSum = innerSum + alpha[m] * dCdX[m] *attmapArray[m]
        #innerSum = innerSum + alpha[m] * dCdX[m]
    # Multiply inner sum by the fitting factor to approximate the sign(.) function
    innerSum = innerSum * fittingFactor
    # Now compute the sech^2 of the inner sum
    ...
    # Now do the final computation to get dX/dAlpha (may not actually need for loop)
    for m in range(0, numModels):
        dXdAlpha[m] = fittingFactor * epsStep * dCdX[m] * innerSumSecSquare * attmapArray[m]
        #dXdAlpha[m] = fittingFactor * epsStep * dCdX[m] * innerSumSecSquare
    # All done so return
    return dXdAlpha
...
def FGSMNativeGradient(device, dataLoader, modelPlus):
    ...
    for xData, yData in sizeCorrectedLoader:
        ...
        elif modelPlus.modelName == 'SpikTransformer_SNN':
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            output = model(xDataTemp)
        elif modelPlus.modelName == 'Fisher SNN':
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            output = sum(model(xDataTemp)) / 10
        else:
            # Forward pass the data through the model
            output = model(xDataTemp)
        # Calculate the loss
...
def GetModelListBestGradient(device, modelList, xK, yK):
    bestloss = 0
    for model in modelList:
        ...
        outputs = model(xK.to(device)).mean(0)  # Spiking Jelly change to average spikes
        model.zero_grad()
        # Compute the loss
...
def GetModelListBestGradientProfiling(device, modelList, xK, yK, estlist=None):
    bestloss = 0
    bestest = 0
...
def GetModelListBestGradientFisher(device, modelList, xK, yK):
    bestloss = 0
    # bestloss = 1000000
    for model in modelList:
        functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly
        ...
        xK.requires_grad = True
        # Pass the inputs through the model
        outputs = model(xK.to(device))  # Spiking Jelly change to average spikes
        outputs = sum(outputs) / 10 # Spiking Jelly change to average spikes
        model.zero_grad()
        # Compute the loss
        cost = loss(outputs, yK)
        cost.backward()
...
def GetModelListBestGradient_BP(device, modelList, xK, yK):
    bestloss = 0
    for model in modelList:
        # Define the loss function
        loss = torch.nn.CrossEntropyLoss()
...
def GetModelListBestGradient_normal(device, modelList, xK, yK):
    bestloss = 0
    for model in modelList:
        # Define the loss function
        loss = torch.nn.CrossEntropyLoss()
...
def FGSMGreedyGradientProfiling(device, dataLoader, modelPlus, modelList, estlist):
    # Basic variable setup
    # model = modelPlus.model
    # model.eval()  # Change model to evaluation mode for the attack
    # model.to(device)
    ...
    for xData, yData in sizeCorrectedLoader:
        ...
        if modelPlus.modelName == 'SNN ResNet Backprop':
            xDataTempGrad = GetModelListBestGradientProfiling(device, modelList, xDataTemp, yData, estlist=estlist)
            # xDataTempGrad = GetModelListBestGradient(device, modelList, xDataTemp, yData, estlist=estlist)
        # elif modelPlus.modelName == 'SpikTransformer_SNN':
        #     functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
        #     output = model(xDataTemp)
        elif modelPlus.modelName == 'SNN VGG-16 Backprop':
            xDataTempGrad = GetModelListBestGradient_BP(device, modelList, xDataTemp, yData)
        elif modelPlus.modelName == 'Fisher SNN':
            xDataTempGrad = GetModelListBestGradientFisher(device, modelList, xDataTemp, yData)
            # functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            # output = sum(model(xDataTemp)) / 10
        else:
            xDataTempGrad = GetModelListBestGradient_normal(device, modelList, xDataTemp, yData)
        #     # Forward pass the data through the model
        #     output = model(xDataTemp)
        # # Calculate the loss
        # loss = torch.nn.CrossEntropyLoss()
...
def FGSMGreedyGradient(device, dataLoader, modelPlus, modelList):
    # Basic variable setup
    # model = modelPlus.model
    # model.eval()  # Change model to evaluation mode for the attack
    # model.to(device)
    ...
    for xData, yData in sizeCorrectedLoader:
        ...
        if modelPlus.modelName == 'SNN ResNet Backprop':
            xDataTempGrad = GetModelListBestGradient(device, modelList, xDataTemp, yData)
        # elif modelPlus.modelName == 'SpikTransformer_SNN':
        #     functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
        #     output = model(xDataTemp)
        elif modelPlus.modelName == 'SNN VGG-16 Backprop':
...
def dFdXCompute(device, dataLoader, modelPlus, confidence, nClasses):
    ...
    for xData, yData in sizeCorrectedLoader:
        ...
        if modelPlus.modelName == 'SNN ResNet Backprop':
            ...
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            outputLogits = sum(model(xDataTemp)) / 10
        else:
            # Forward pass the data through the model
            outputLogits = model(xDataTemp)
...
# Do the computation from scratch to get the correctly identified overlapping examples
# Note these samples will be the same size as the input size required by the 0th model
def GetFirstCorrectlyOverlappingSamplesBalanced(device, sampleNum, numClasses, dataLoader, modelPlusList, imgNet=False):
    numModels = len(modelPlusList)
    totalSampleNum = len(dataLoader.dataset)
    # # First check if modelA needs resize
    # xTestOrig, yTestOrig = DMP.DataLoaderToTensor(dataLoader)
    ...
    # #                                         batchSize=dataLoader.batch_size, randomizer=None)
    # Get accuracy array for each model
    accArrayCumulative = torch.zeros(totalSampleNum).to(device)  # Create an array with one entry for every sample in the dataset
    if imgNet:
        for i in range(0, numModels):
    ...
    # Memory for the solution
    # xClean = torch.zeros(sampleNum, 3, modelPlusList[0].imgSizeH, modelPlusList[0].imgSizeW)
    xClean = torch.zeros(sampleNum, 3, xTest.shape[2], xTest.shape[3])
    yClean = torch.zeros(sampleNum)
    sampleIndexer = 0
    # Go through all the samples
    for i in range(0, totalSampleNum):
...
def DIFGSMGradient(device, dataLoader, modelPlus, decay, resize_rate, diversity_prob):
    # Basic variable setup
    model = modelPlus.model
    model.eval()  # Change model to evaluation mode for the attack
    model.to(device)
    ...
    for xData, yData in sizeCorrectedLoader:
        ...
        # Set requires_grad attribute of tensor. Important for attack. (Pytorch comment, not mine)
        xDataTemp.requires_grad = True
        xAdvCurrent = input_diversity(xDataTemp, resize_rate, diversity_prob)
        if modelPlus.modelName == 'SNN ResNet Backprop':
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            # Forward pass the data through the model
            output = model(xAdvCurrent).mean(0)
        elif modelPlus.modelName == 'SpikTransformer_SNN':
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            output = model(xAdvCurrent)
        elif modelPlus.modelName == 'Fisher SNN':
            functional.reset_net(model)  # Line to reset model memory to accodomate Spiking Jelly (new attack iteration)
            output = sum(model(xAdvCurrent)) / 10
        else:
            # Forward pass the data through the model
            output = model(xAdvCurrent)
        # Calculate the loss
        loss = torch.nn.CrossEntropyLoss()
        momentum = torch.zeros_like(xData).detach().to(device)
        # Zero all existing gradients
        model.zero_grad()
        # Calculate gradients of model in backward pass
        cost = loss(output, yData).to(device)
        ...
        xDataTempGrad = torch.autograd.grad(
            cost, xDataTemp, retain_graph=False, create_graph=False
        )[0]
        if modelPlus.modelName == 'SNN VGG-16 Backprop':
            xDataTempGrad = xDataTempGrad.sum(-1)

        xDataTempGrad = xDataTempGrad / torch.mean(torch.abs(xDataTempGrad), dim=(1, 2, 3), keepdim=True)
        xDataTempGrad = xDataTempGrad + momentum * decay
        momentum = xDataTempGrad
        # Save the adversarial images from the batch
        for j in range(0, batchSize):
            xGradient[advSampleIndex] = xDataTempGrad[j]
            yClean[advSampleIndex] = yData[j]
...
def EnserbleGradientAttack(device, epsMax, numSteps, epsStep, modelListPlus, coefficientArray, dataLoader, clipMin,
                                clipMax, decay=0.0, resize_rate=0.9, diversity_prob=0.5):
    # Basic graident variable setup
    xClean, yClean = DMP.DataLoaderToTensor(dataLoader)
    xAdv = xClean.to(device).detach()  # Set the initial adversarial samples
    numSamples = len(dataLoader.dataset)  # Get the total number of samples to attack
    ...
    for i in range(0, numSteps):
        ...
        for m in range(0, len(modelListPlus)):
            ...
            # First get the gradient from the model
            # dataLoaderCurrent = currentModelPlus.formatDataLoader(dataLoaderCurrent)
            xGradientCurrent = DIFGSMGradient(device, dataLoaderCurrent, currentModelPlus, decay, resize_rate, diversity_prob)
            # Resize the graident to be the correct size
            xGradientCurrent = torch.nn.functional.interpolate(xGradientCurrent, size=(xShape[1], xShape[2]))
            # Add the current computed gradient to the result
            xGradientCumulative = xGradientCumulative + coefficientArray[m] * xGradientCurrent
