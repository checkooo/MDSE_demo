...
class threshold_erfc(torch.autograd.Function):
    """
    ...
    def backward(ctx, grad_output):
        ...
        exponent = -torch.pow(input, 2)/0.32
        exp = torch.exp(exponent)

        erfc_grad = exp / 1.0026513098524001
...
class threshold_slayer(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        spikePdf = 1/10 * torch.exp( -torch.abs(membranePotential) / 10)

        return grad_output * spikePdf, None
...
class threshold_fastsigmoid(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        """
        input, = ctx.saved_tensors
        denominator = (torch.abs(input) + 1)**2

        grad_input = grad_output.clone()
        return grad_input / denominator, None
...
class VGG_SNN_STDB(nn.Module):
    ...
    def __init__(self, vgg_name, activation='Arctan', labels=10, timesteps=100, leak=1.0, default_threshold=1.0,
                 dropout=0.2, kernel_size=3, dataset='CIFAR10', mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010]):
        super().__init__()

        self.vgg_name = vgg_name
        if activation == 'Linear':
            self.act_func = LinearSpike.apply
        elif activation == 'Erfc':
            self.act_func = threshold_erfc.apply
        elif activation == 'Logistic':
            self.act_func = threshold_logistic.apply
        elif activation == 'Slayer':
            self.act_func = threshold_slayer.apply
        elif activation == 'ActFun':
            self.act_func = ActFun.apply
        elif activation == 'Fastsigmoid':
            self.act_func = threshold_fastsigmoid.apply
        elif activation == 'Arctan':
            self.act_func = threshold_arctan.apply
    ...
    def _make_layers(self, cfg):
        ...
        elif (self.vgg_name != 'VGG5' and self.vgg_name != 'VGG16' and self.vgg_name != 'VGG11') and self.dataset != 'MNIST':
        ...
        elif self.vgg_name == 'VGG11' and self.dataset != 'MNIST':
