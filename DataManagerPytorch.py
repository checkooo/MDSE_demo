...
def GetCIFAR10Train_norm(imgSize = 32, batchSize=128, mean=(0.4914, 0.4822, 0.4465), std=(0.2023, 0.1994, 0.2010)):
    transformTest = transforms.Compose([
        transforms.Resize((imgSize, imgSize)),
        transforms.ToTensor(),
        # transforms.Normalize(mean=[mean, mean, mean], std=[0.5,0.5,0.5]),
    ...
    valLoader = torch.utils.data.DataLoader(datasets.CIFAR10(root='./data', train=True, download=True, transform=transformTest), batch_size=batchSize, shuffle=False, num_workers=0, pin_memory=False)
    return valLoader
