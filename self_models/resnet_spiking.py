...
class RESNET_SNN_STDB(nn.Module):
	...
	def neuron_init(self, x):
		...
		for l in range(len(self.pre_process)):
			...
			if isinstance(self.pre_process[l], nn.Conv2d):
				self.mem[l] = torch.zeros(self.batch_size, self.pre_process[l].out_channels, self.width, self.height).cuda()

			elif isinstance(self.pre_process[l], nn.Dropout):
				self.mask[l] = self.pre_process[l](torch.ones(self.mem[l-2].shape).cuda())
		...
		for i in range(1,5):
			...
			for index in range(len(layer)):
				for l in range(len(layer[index].residual)):
					if isinstance(layer[index].residual[l],nn.Conv2d):
						self.mem[pos] = torch.zeros(self.batch_size, layer[index].residual[l].out_channels, self.width, self.height).cuda()
						pos = pos + 1
					elif isinstance(layer[index].residual[l],nn.Dropout):
						self.mask[pos-1] = layer[index].residual[l](torch.ones(self.mem[pos-1].shape).cuda())
		...
		self.mem[pos] = torch.zeros(self.batch_size, self.classifier[0].out_features).cuda()
