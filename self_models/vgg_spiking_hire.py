...
class VGG_SNN_STDB(nn.Module):
    ...
    def forward(self, x, init_flag, n_period, stage, find_max_mem=False, max_mem_layer=0):
        if init_flag:
            self.neuron_init(x)
            max_mem = 0.0
        else:
            copymem = self.mem
            copyspike = self.spike
            copymask = self.mask
            self.mem = {}
            self.spike = {}
            self.mask = {}
            keys = copymem.keys()
            for key in keys:
                self.mem[key] = copymem[key].detach()
            keys = copyspike.keys()
            for key in keys:
                self.spike[key] = copyspike[key].detach()
            keys = copymask.keys()
            for key in keys:
                self.mask[key] = copymask[key].detach()
        ann = [0] * len(self.features)
        # ann[1] = 1
        # pdb.set_trace()
        start = int(self.timesteps/n_period * stage)
        end = int(self.timesteps/n_period * (stage+1))
