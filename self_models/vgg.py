...
class VGG(nn.Module):
    def __init__(self, vgg_name='VGG16', labels=10, dataset = 'CIFAR10', kernel_size=3, dropout=0.2, mean=[0.5071, 0.4867, 0.4408], std=[0.2675, 0.2565, 0.2761], imgSize=32):
        ...
        if vgg_name == 'VGG5' and dataset == 'MNIST':
            ...
            if imgSize == 128:
                self.classifier = nn.Sequential(
                                ...
                                # nn.ReLU(inplace=True),
                                # nn.Dropout(self.dropout),
                                nn.Linear(8192, labels, bias=False)
